...
int main()
{
	...
	while (true)
	{
		...
		if (!GF.nextstep(bl)) return 0; //выходим из программы, если избашня из блоков "доросла до верха"
		GF.combustion();
...
//Данный код представляет собой упрощенную реализацию игры Тетрис.Вот краткое объяснение его структуры и функциональности :

//В коде подключены необходимые заголовочные файлы для работы с вводом / выводом, векторами, системными функциями и т.д.
//Есть две глобальные переменные : var_blocks(вектор строк, представляющих различные конфигурации блоков) и Field(двумерный вектор, представляющий игровое поле).
//В коде определены два класса : Block и GameField.
//Класс Block представляет отдельный блок в игре и имеет переменные и функции, связанные с его свойствами и поведением.
//Класс GameField представляет игровое поле и содержит переменные и функции для управления состоянием игры и отображения поля.
//Функция main является точкой входа в игру.
//Внутри функции main создается экземпляр класса GameField и динамически выделяется память под блок.
//Главный игровой цикл выполняется до окончания игры.
//Внутри цикла игры захватывается пользовательский ввод с помощью функций _kbhit и _getch, что позволяет игроку перемещать и вращать блок.
//Функция nextstep перемещает блок вниз на один шаг и проверяет наличие столкновений или конец игры.
//Функция visGameField отображает текущее состояние игрового поля, включая падающий блок.
//Функция combustion проверяет наличие заполненных линий на поле, очищает их, корректирует счет и сдвигает оставшиеся блоки вверх.
//В коде присутствуют вспомогательные функции NormLeft и NormRight, которые проверяют возможность перемещения блока влево или вправо без столкновений.
//Функция turn в классе Block осуществляет поворот блока по часовой стрелке, если нет столкновений.
//Функция THEEND отображает итоговый счет при завершении игры.
//В игровом цикле присутствует задержка с помощью функции Sleep, чтобы контролировать скорость падающего блока.
//В целом, код представляет собой базовую реализацию игры Тетрис с ограниченным набором функций и пользовательского взаимодействия.